...
export function uniqueAndCommonNodes(
  xs: any[],
...
function get_index(genebanks: Genebank[]): Record<string, Individual> {
  if (!indexes.has(genebanks)) {
    ...
    genebanks.forEach((genebank) =>
      (genebank.individuals || []).forEach((i) => {
        index[i.number] = i;
      })
    );
...
const asNode = (ind: Individual, x: number): Node => {
  ...
  return {
    ...
    shape:
      ind.sex == "male" ? "box" : ind.sex == "female" ? "oval" : "triangle", // unknown sex
    color: color,
  };
};
...
export function herdPedigree(
  ...
  const genebank = genebanks.find((genebank) =>
    genebank.herds.some((herd) => herd.herd == herdId)
  );
  if (!genebank || !genebank.individuals) {
    return { nodes: [], edges: [] };
  }

  const herd = genebank.individuals.filter(
    (individual) => individual.herd.herd == herdId
  );
  ...
  herd.forEach((individual) => {
    const pedigree = (algo === "Martin" ? calcPedigree : calcPedigreeDan)(
      genebanks,
      individual.number,
      generations
    );
 ...
 */
export function getConnectingEdges(
  edges: Edge[],
  ancestorId: string,
  seekedDescendantId: string
): Set<string> {
  const getChildren = (
    edges: Edge[],
    parentId: string,
...
function ancestorsPedigree(
  genebanks: Genebank[],
  parents: LimitedIndividual[],
  offspringId: string,
  generations: number
) {
  let nodes: Node[] = [];
 ...
 * @returns
 */
export function testBreedPedigree(
  genebanks: Genebank[],
  chosenAncestors: testBreedIndividuals,
  generations: number = 1000,
  showCommonAncestors: boolean
): { pedigree: Pedigree; commonAncestors: boolean } {
  let nodes: Node[] = [];
  let edges: Edge[] = [];
  ...
  if (chosenAncestors["male"]) {
    maleAncestorsPedigree = ancestorsPedigree(
      genebanks,
      [chosenAncestors["male"]],
      offspringNode.id,
      generations - 1
    );
  } else {
    let offspringFather = pseudoNode(
      "1337133711",
      "Ej registrerad\nfar",
      "box",
      "#d4ecfc"
    ...
    edges.push({
      id: `${offspringNode.id}-${offspringFather.id}`,
    ...
    maleAncestorsPedigree = ancestorsPedigree(
      genebanks,
      [chosenAncestors["maleGF"], chosenAncestors["maleGM"]],
      offspringFather.id,
      generations - 2
    );
  }
  ...
  if (chosenAncestors["female"]) {
    femaleAncestorsPedigree = ancestorsPedigree(
      genebanks,
      [chosenAncestors["female"]],
      offspringNode.id,
      generations - 1
    );
  } else {
    let offspringMother = pseudoNode(
      "1337133700",
      "Ej registrerad\nmor",
      "oval",
      "#fce5e9"
    ...
    edges.push({
      id: `${offspringNode.id}-${offspringMother.id}`,
      from: offspringNode.id,
      to: offspringMother.id,
    });
    femaleAncestorsPedigree = ancestorsPedigree(
      genebanks,
      [chosenAncestors["femaleGF"], chosenAncestors["femaleGM"]],
      offspringMother.id,
      generations - 2
    );
  }
  nodes = [
    ...nodes,
    ...femaleAncestorsPedigree.nodes,
    ...maleAncestorsPedigree.nodes,
  ];
  edges = [
    ...edges,
    ...femaleAncestorsPedigree.edges,
    ...maleAncestorsPedigree.edges,
  ];
  ...
  if (showCommonAncestors) {
    ...
    commonNodes.forEach((nodeId: string) => {
      let ancestorEdges = getConnectingEdges(
        uniqueEdges,
        nodeId,
        offspringNode.id
      );
  ...
  return {
    pedigree: { nodes: uniqueNodes, edges: uniqueEdges },
    commonAncestors: commonNodes.size > 0,
  };
}
