...
export function useRoutedTabs(
  ...
  const set_tab = (
    index: number | ((index: number, num_tabs: number) => number)
  ) => {
    const next_index =
      typeof index == "function" ? index(tab_index, routed_tabs.length) : index;
...
export function Routed(props: {
  ...
  return (
    <Route path={props.path}>
      <RoutedInner>{props.children}</RoutedInner>
    </Route>
  );
}
