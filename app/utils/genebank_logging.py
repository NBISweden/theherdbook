...
def create_genebank_logs(first_path, first_log_name):
    for log_type in "update","create":
        ...
        if not fileExists:
            if log_type == "update":
                handler.configureHeaderWriter('datum,användare,individ.number,fält,gammalt,nytt,sälj/rapport-datum', logger)
            if log_type == "create":
                handler.configureHeaderWriter('datum,användare,individ.number,individ.namn,besättning,sälj-datum', logger)
