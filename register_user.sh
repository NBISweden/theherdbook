...
then
	...
	USAGE: ./register_user.sh <user> <pass> <email>
	END_ERROR
	exit 1
fi
...
docker-compose exec -T main python3 -c "from utils.data_access import register_user; register_user('${email}', '${pass}', '${user}', privileges=[{'level': 'admin'}])"
